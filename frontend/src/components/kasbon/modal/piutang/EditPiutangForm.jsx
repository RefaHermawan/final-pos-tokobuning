// src/components/kasbon/modal/EditPiutangForm.jsx
import React, { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import apiClient from '../../../../api/axios';
import { useToast } from '../../../../hooks/useToast';
import Modal from '../../../ui/Modal';// Pastikan path ini benar
import { Save, X, Coins, CalendarDays, Edit as EditIcon, User } from 'lucide-react';

const EditPiutangForm = ({ isOpen, onClose, onSuccess, kasbonItem }) => {
  const [formData, setFormData] = useState({ nama_pelanggan: '', total_awal: '', tanggal_jatuh_tempo: '' });
...
      setFormData({
        nama_pelanggan: kasbonItem.pelanggan_nama || '',
...
      await apiClient.patch(`/transactions/hutang-piutang/${kasbonItem.id}/`, payload);
      addToast("success", <>Piutang untuk <span className="font-bold text-accent">{formData.pelanggan_nama}</span> berhasil diperbarui.</>);
      onSuccess();
    } catch (err) {
      const errorMessage = err.response?.data?.detail || "Gagal mengupdate data piutang.";
        ...
        >
            <div className="flex items-center justify-between p-4 border-b border-light-gray/50">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-bold text-text-title">Edit Piutang</h2>
                        <p className="text-xs text-text-secondary">Untuk Pelanggan: <span className="font-semibold text-accent">{kasbonItem?.pelanggan_nama}</span></p>
                    </div>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6">
                <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-5">
                    ...
                    <motion.div variants={itemVariants}>
                        ...
                        <div className="relative">
                            ...
                            <input name="pelanggan_nama" type="text" value={formData.nama_pelanggan} onChange={handleChange} required 
                    ...
                    <motion.div variants={itemVariants}>
                        <label className="text-sm font-semibold text-text-main mb-2 block">Jumlah Total Baru {required_field}</label>
                        <div className="relative">
                            <span className="absolute left-4 top-1/2 -translate-y-1/2 font-semibold text-lg text-text-secondary">Rp</span>
                            <input name="total_awal" type="number" value={formData.total_awal} onChange={handleChange} required 
                    ...
                    <motion.div variants={itemVariants}>
                        <label className="text-sm font-semibold text-text-main mb-2 block">Tgl Jatuh Tempo <span className="font-normal text-text-secondary">(Opsional)</span></label>
                         <div className="relative">
                            <input name="tanggal_jatuh_tempo" type="date" value={formData.tanggal_jatuh_tempo} onChange={handleChange} 
                ...
                <div className="flex justify-end items-center gap-4 pt-6 mt-6 border-t border-light-gray/50">
                    ...
                    <motion.button type="submit" disabled={isSaving} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center gap-2 px-5 py-2.5 text-sm font-bold text-white bg-accent rounded-lg shadow-lg hover:shadow-accent/50 disabled:opacity-60">
                        <Save size={16} />{isSaving ? "Menyimpan..." : "Update Piutang"}
                    </motion.button>
                </div>
            </form>
        </motion.div>
...
export default EditPiutangForm;